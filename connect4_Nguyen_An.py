...
def simulate(board, player, policy='random'):
    ...
    while not terminal_node(temp_board):
        ...
        if policy == 'random':
            ...
            move = None
            for collumn in valid_moves:
                row = get_next_open_row(temp_board,collumn)
                test_board = temp_board.copy()
                drop_piece(test_board,row,collumn,current_player)
                if winning_move(test_board,current_player):
                    move = collumn
                    break
            if move is None:
                    ...
                    for collumn in valid_moves:
                        row = get_next_open_row(temp_board,collumn)
                        test_board = temp_board.copy()
                        drop_piece(test_board,row,collumn,opponent)
...
def montecarlo(root_board, simulations, policy = 'smart'):
    root = create_node(root_board.copy(), player=1)
    for i in range(simulations):
        node = select(root)
        next_player = 1 if node['player'] == 2 else 2
        winner = simulate(node['board'], next_player, policy)
        backpropagate(node, winner)
    best_child = max(root['children'], key=lambda c: c['visits'])
    with open("mcts_tree_snapshot.txt", "w") as f:
        for child in root['children']:
            if child['visits'] > 0:
                f.write(f"Move {child['move']} -> Wins: {child['wins']} / Visits: {child['visits']}\n")
...
while not game_over:
    ...
    if turn == 1 and not game_over:
        starttime = time.time()
        if GameModes == 'mtcs':
            collumn = montecarlo(board, 250)
            endtime = time.time()
            print("Move Time:", endtime - starttime)
        elif GameModes == 'AB':
            starttime = time.time()
            collumn, score = minimax(board, 6, -math.inf, math.inf, True)
            endtime = time.time()
            print("Move Time:", endtime - starttime)
        if is_valid_location(board,collumn):
            row = get_next_open_row(board, collumn)
